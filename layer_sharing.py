...
# The FL_* family is shared with fedagent.py so a single comparison harness can
# align both runs (see compare.py).
...
COUNT_THRESHOLD = 5                                          # consecutive stable rounds
# When FL_DISABLE_EARLY_STOP=1 we push MINIMUM_ROUNDS past R_PRIME so the
# convergence-based termination check never fires and the run uses exactly
# R_PRIME rounds (matches fedagent.py's fixed-T schedule for fair comparison).
MINIMUM_ROUNDS = (
...
def parse_input_file():
    """Reads ``inputf.txt`` unless ``LAYER_SHARING_INPUT`` is set (e.g. for localhost tests)."""
